...
std::vector<std::string> MeshModel::loadMaterials(const aiScene* scene)
{
	...
	for (size_t i = 0; i < scene->mNumMaterials; i++)
	{
		...
		{
			// Get the path of the texture file (glTF often uses "textures/foo.jpeg" relative to model)
			aiString path;
			if (material->GetTexture(aiTextureType_DIFFUSE, 0, &path) == AI_SUCCESS)
			{
				std::string pathStr(path.data);
				// Keep full relative path so MoonObject/scene.gltf -> "textures/Material.002_diffuse.jpeg" loads from MoonObject/textures/
				textureList[i] = pathStr;
			}
		}
	}
